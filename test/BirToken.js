...
it('handles delegated token transfer',function(){
		return BirToken.deployed()
			.then(function(instance){
				...
				assert(error.message.indexOf('revert') >=0,'cannot transfer value larger than amount ot balance');		
				// try something latger than the apprpved amount
				return tokenInstance.transferFrom(fromAccount,toAccount,7,{from:spendingAccount});	
			}).then(assert.fail).catch(function(error){
				assert(error.message.indexOf('revert')>=0,'cannot transfer value larger than the approved ammout');
				...
				assert.equal(balance.toNumber(),5,'correct amount was deducted');
				return tokenInstance.balanceOf(toAccount);
			}).then(function(balance){
				assert.equal(balance.toNumber(),5,'correct amount was added');
				return tokenInstance.allowance(fromAccount,spendingAccount);
			}).then(function(allowance){
				assert.equal(allowance.toNumber(),0,'the allowance amount was fully ussed');
			});

				
});
		
// contnue fron 2:38:00 min // errot  on adds the amoynt ffrom the receibinf g axxount 
});
